# ==============================================================================
# 01a_risk_set_adoption.R
#
# Builds the dyadic ADOPTION risk set.
#
# LOGIC:
#   (i, j, s) | RCA_i(s,t0) > 1 & RCA_j(s,t0) <= 1
#   Outcome: does occupation j gain specialization in skill s between t0 and t1?
#
# FIXES:
...
#   Fix 4: RCA denominator-drift robustness — RCA(j,s) is recomputed each year
#          from the FULL economy-wide matrix (b_s = st/gt). Because RCA is a
#          Balassa-style index, its weighted mean across occupations equals 1
#          by construction: if high-status occupations raise their reliance on
#          a skill, the global bar (b_s) rises for everyone else, and an
#          occupation can cross below RCA=1 with zero change in its own
#          behavior. Fix 4 carries two parallel outcome measures alongside the
#          standard RCA threshold:
#            (a) diffusion_fixed_denom — RCA with b_s frozen at its 2015
#                value, so only the occupation's own behavior (a_js) can
#                move it across the threshold
#            (b) delta_importance — raw O*NET importance change (v_t1 - v_t0),
#                which carries no relative/matrix-wide normalization at all
#   Fix 5: skill Element.Name harmonization across O*NET releases. O*NET
#          revises the wording of some Work Activities/Knowledge element
#          labels between releases (e.g. "Operation Monitoring" ->
#          "Operations Monitoring"). Without harmonization, the Step 4 merge
#          (by soc_code, skill) treats the same construct as two different
#          skills: every 2015-specialized occupation appears to ABANDON the
#          old label, and every 2024-specialized occupation appears to freshly
#          ADOPT the new label. This contaminates BOTH risk sets for the
#          affected skills, not just the Fix 4 placebo. 8 of 160 skills were
#          affected as of this version (identified via
#          00_check_missing_skills.R); see the harmonization map below.
#
# Input:  data/raw/onet/db_15_1/
#         data/raw/onet/db_29_2_text/
#         data/crosswalk/2010_to_2019_Crosswalk.csv
#
# Output: data/derived/all_events_adoption.rds
#         Columns: source, target, skill_name, diffusion,
#                  diffusion_fixed_denom, delta_importance,
#                  domain, structural_distance, zero_overlap
#
# Intermediates: deleted before final save
# Next:          02a_enrich_adoption.R (must propagate diffusion_fixed_denom
#                and delta_importance through to the modeling stage)
# ==============================================================================
...
#
# Mapping built from 00_check_missing_skills.R fuzzy-match output (2024 name
# -> 2015 name). 2015 is treated as the canonical label, consistent with the
# rest of the pipeline (domain classification, status index, and skill
# taxonomy are all predetermined from 2015 data).
#
# The "Administrative" / "Clerical" pair has a high edit distance (12) and
# was NOT confirmed by string similarity alone. It is the only pair left over
# among the 8 orphaned names on each side, which is suggestive, but VERIFY
# against O*NET's official Content Model change documentation before
# trusting it.
# ==============================================================================
skill_name_harmonization <- c(
  ...
  "Administrative"                                        = "Clerical"  # VERIFY against O*NET docs
)
...
# Fix 4: calc_rca() exposes a_js (within-occupation share, the REAL
# behavioral signal) and b_s (economy-wide share, the term that drifts)
# as separate columns, not just their ratio.
calc_rca <- function(d) {
...
# Detect Scale ID column robustly
...
# --- Fix 5: harmonize 2024 Element.Name to 2015 convention BEFORE any RCA
#     computation, so the Step 4 merge by `skill` doesn't silently split a
#     single renamed construct into a spurious abandon + spurious adopt. ---
...
# Verify Fix 5 worked: zero orphaned skill names in either direction
...
if (orphan_check > 0) {
  warning(sprintf(
    "  [!!] %d skill names still don't match between 2015 and 2024 after harmonization. Run 00_check_missing_skills.R again to identify them.",
    orphan_check))
} else {
...
#
# b_s (the economy-wide term) is recomputed from the full 2024 matrix, so an
# occupation can cross RCA>1 purely because OTHER occupations raised their
# reliance on skill s, with zero change in its own behavior. This freezes
# b_s at its 2015 value while letting a_js (the occupation's own behavior)
# update normally from 2024 data — isolating whether threshold crossings
# survive when the bar itself doesn't move.
# ==============================================================================
message("\n>>> Fix 4: Fixed-denominator RCA placebo...")
...
# Quick diagnostic: how much does freezing the denominator change things?
rca_compare <- merge(
...
# ==============================================================================
# Step 4 — Panel and ADOPTION dyad construction
# ==============================================================================
message("\n>>> Step 4: Adoption risk set...")
...
# With Fix 5 applied, this count should now be ~0 (vs. nontrivial before):
# rows where one side of (soc_code, skill) has no counterpart at all are
# occupations genuinely entering/leaving the SOC taxonomy, not renamed skills.
...
# Sources: RCA>1 at t0 (have the skill)
sources_t0 <- panel[rca_t0 > 1, .(source = soc_code, skill)]

# Targets: RCA<=1 at t0 (can adopt)
...
targets_risk[, diffusion             := fifelse(rca_t1_tgt > 1, 1L, 0L)]
targets_risk[, diffusion_fixed_denom := fifelse(rca_t1_fixed_tgt > 1, 1L, 0L)]
...
message(sprintf("  Estimated risk set: ~%s triads",
                format(sum(n_est$n_pairs), big.mark = ",")))
message(sprintf("  Expected diffusion rate (standard RCA):       %.4f",
                targets_risk[, mean(diffusion)]))
message(sprintf("  Expected diffusion rate (fixed denominator):  %.4f",
                targets_risk[, mean(diffusion_fixed_denom)]))
...
# ==============================================================================
FLUSH_EVERY <- 20L
chunk_dir   <- file.path(output_data_dir, "_adopt_chunks")
...
for (sk in skills_vec) {
  ...
  pairs <- merge(
    pairs,
    tgt_sk[, .(target, diffusion, diffusion_fixed_denom, delta_importance)],
    by = "target"
  )
  pairs[, skill_name := sk]
...
message(sprintf("  Total adoption dyads: %s",
                format(nrow(all_events), big.mark = ",")))
message(sprintf("  Diffusion rate (standard):       %.4f",
                mean(all_events$diffusion)))
message(sprintf("  Diffusion rate (fixed denom.):    %.4f",
                mean(all_events$diffusion_fixed_denom)))
...
message(sprintf("  Diffusion rate (standard, RCA threshold):  %.4f",
                mean(all_events$diffusion)))
message(sprintf("  Diffusion rate (fixed-denominator placebo): %.4f",
                mean(all_events$diffusion_fixed_denom)))
message(sprintf("  Correlation, standard vs. fixed-denom outcome: %.4f",
                cor(all_events$diffusion, all_events$diffusion_fixed_denom)))
message(sprintf("  Mean raw importance delta:                  %.4f",
                mean(all_events$delta_importance)))

message("\n  --- By domain ---")
print(all_events[, .(
  rate_standard   = mean(diffusion),
  rate_fixed      = mean(diffusion_fixed_denom),
  mean_raw_delta  = mean(delta_importance),
  n               = .N
), by = domain])

out_path <- file.path(output_data_dir, "all_events_adoption.rds")
saveRDS(all_events, out_path)
message(sprintf("\n  Saved: %s", out_path))
message(sprintf("  Size:  %.1f MB", file.size(out_path) / 1e6))

message("\n  NOTE: 'diffusion' (standard RCA) remains the primary outcome for")
message("  the main models. 'diffusion_fixed_denom' and 'delta_importance' are")
message("  carried forward for the SI robustness check (RCA denominator-drift)")
message("  and must be propagated through 02a_enrich_adoption.R and the gravity")
message("  model script as alternative dependent variables.")

rm(all_events); gc()
message("\n>>> 01a_risk_set_adoption.R complete.")
message("    Next: 02a_enrich_adoption.R")