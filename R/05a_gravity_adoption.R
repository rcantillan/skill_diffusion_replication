# ==============================================================================
# 05a_gravity_adoption.R
#
# Cloglog gravity models for ADOPTION using the PC1 status index.
#
# DESIGN:
#   source i = occupation with RCA>1 at t0 (carries the skill)
#   target j = occupation with RCA<=1 at t0 (can adopt)
#   outcome  = diffusion_j = 1 if j gains RCA>1 between t0 and t1
#   gap      = G_ij = status_j - status_i (positive = j has higher status)
#
# STATUS:
#   status_pc1 per occupation from occ_status_scores.csv
#   G_ij = status_target - status_source  (level difference)
#
# IDENTIFICATION:
...
#   SC archetypes:     beta_up > 0  (higher-status targets adopt more)
#   Physical_Terminal: beta_up < 0  (barrier: physical resists upward movement)
#
...
FORCE_REFIT <- TRUE   # TRUE = always re-estimate; FALSE = load if exists

# Paths
dt_path    <- if (exists("PATH_ADOPTION")) PATH_ADOPTION else
...
stopifnot("riskset_adoption.rds not found"     = file.exists(dt_path))
...
message(sprintf("PC1: %.1f%% var | flip applied: %s | FORCE_REFIT: %s",
                pca_dec$pc1_pct_var,
...
dt <- readRDS(dt_path)
setDT(dt)
stopifnot("diffusion"  %in% names(dt))
stopifnot("cs"         %in% names(dt))
stopifnot("domain"     %in% names(dt))
message(sprintf("  Rows: %s | Diffusion rate: %.4f",
                format(nrow(dt), big.mark = ","),
                mean(dt$diffusion)))

# Reduce to needed columns
KEEP <- c("source", "target", "skill_name", "diffusion",
...
dt <- dt[, .(source, target, skill_name, diffusion,
             atc_archetype, structural_distance)]
...
message(sprintf("  Scores loaded: %d occupations", nrow(scores)))
message(sprintf("  status_pc1: range [%.3f, %.3f]",
                min(scores$status_pc1), max(scores$status_pc1)))
...
n_na_gap <- dt[is.na(status_source) | is.na(status_target), .N]
message(sprintf("  Dyads without status: %d (%.2f%%)",
                n_na_gap, 100 * n_na_gap / nrow(dt)))
dt <- dt[!is.na(status_source) & !is.na(status_target)]

# G_ij = status_target - status_source
# > 0: target j has HIGHER status (upward)
# < 0: target j has LOWER status (downward)
...
message(sprintf("  Upward dyads (G_ij>0): %.1f%%",
                100 * mean(dt$pc1_dummy)))

# Preliminary DTC diagnostic
tab <- dt[, .(rate = mean(diffusion), n = .N),
          by = .(atc_archetype, direction = fifelse(pc1_dummy==1, "up", "down"))]
setorder(tab, atc_archetype, direction)
message("\n  DTC diagnostic (adoption):")
print(tab)
...
# ==============================================================================
# Step 3 — Rug segments for figure
# ==============================================================================
message("\n>>> Step 3: Rug segments...")

rug_full <- dt[, .(
  x       = pc1_up + pc1_down,   # reconstructed status_pc1
...
saveRDS(rug_samp, file.path(out_models, "rug_segs_adoption.rds"))
message("  Saved: rug_segs_adoption.rds")
...
path_A <- file.path(out_models, "model_adoption_panelA.rds")
...
path_B <- file.path(out_models, "model_adoption_panelB.rds")
...
print(coefs[var %in% c("b_up","b_dn")][
  order(panel, archetype, var)])
...
fwrite(coefs,  file.path(out_tables, "coefs_pc1_adoption.csv"))
fwrite(atc_dt, file.path(out_tables, "atc_pc1_adoption.csv"))
message("\n  Saved: coefs_pc1_adoption.csv | atc_pc1_adoption.csv")

gc()
message("\n>>> 05a_gravity_adoption.R complete.")
message("    Next: 05b_gravity_abandonment.R")
